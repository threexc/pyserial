...
class Test_exclusive(unittest.TestCase):
    ...
    def setUp(self):
        with pyserial.serial_for_url(PORT, do_not_open=True) as x:
            if not isinstance(x, pyserial.Serial):
    ...
    def test_exclusive_none(self):
        ...
        with pyserial.Serial(PORT, exclusive=None):
            pass  # OK
    ...
    def test_exclusive_false(self):
        ...
        with pyserial.Serial(PORT, exclusive=False):
            pass  # OK
    ...
    def test_exclusive_true(self):
        ...
        with pyserial.Serial(PORT, exclusive=True):
            with self.assertRaises(pyserial.SerialException):
                pyserial.Serial(PORT, exclusive=True)  # fails to open twice
    ...
    def test_exclusive_only_true(self):
        ...
        with self.assertRaises(ValueError):
            pyserial.Serial(PORT, exclusive=False) # expected to fail: False not supported
