...
class Test_Readline(unittest.TestCase):
    ...
    def test_readline(self):
        ...
        self.s.write(pyserial.to_bytes([0x31, 0x0a, 0x32, 0x0a, 0x33, 0x0a]))
        self.assertEqual(self.s.readline(), pyserial.to_bytes([0x31, 0x0a]))
        self.assertEqual(self.s.readline(), pyserial.to_bytes([0x32, 0x0a]))
        self.assertEqual(self.s.readline(), pyserial.to_bytes([0x33, 0x0a]))
        # this time we will get a timeout
        self.assertEqual(self.s.readline(), pyserial.to_bytes([]))
    ...
    def test_readlines(self):
        ...
        self.assertEqual(
                self.s.readlines(),
                [pyserial.to_bytes([0x31, 0x0a]), pyserial.to_bytes([0x32, 0x0a]), pyserial.to_bytes([0x33, 0x0a])]
                )
    ...
    def test_alternate_eol(self):
        ...
        if hasattr(self.s, 'xreadlines'):  # test if it is our FileLike base class
            self.s.write(pyserial.to_bytes("no\rno\nyes\r\n"))
            self.assertEqual(
                    self.s.readline(eol=pyserial.to_bytes("\r\n")),
                    pyserial.to_bytes("no\rno\nyes\r\n"))
